...
def quick_start(model, dataset, config_dict, save_model=True, mg=False,log_name=''):
    # merge config dict
    config = Config(model, dataset, config_dict, mg)
    init_logger(config,log_name)
    logger = getLogger()
    # print config infor
    logger.info('██Log For: \t' + log_name)
    ...
    for hyper_tuple in combinators:
        ...
        model = get_model(config['model'])(config, train_data,logger).to(config['device'])
        logger.info(model)
