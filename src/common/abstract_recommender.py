...
class GeneralRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataloader):
        ...
        # model:TEST
        if config['video_feature_file']:
            self.video_feat, self.frame_feat = None, None
            if not config['end2end'] and config['is_multimodal_model']:
                dataset_path = os.path.abspath(config['data_path'] + config['dataset'])
                # if file exist?
                v_feat_file_path = os.path.join(dataset_path, config['video_feature_file'])
                t_feat_file_path = os.path.join(dataset_path, config['frame_feature_file'])
                if os.path.isfile(v_feat_file_path):
                    self.video_feat = torch.from_numpy(np.load(v_feat_file_path, allow_pickle=True)).type(
                        torch.FloatTensor).to(
                        self.device)
                if os.path.isfile(t_feat_file_path):
                    self.frame_feat = torch.from_numpy(np.load(t_feat_file_path, allow_pickle=True)).type(
                        torch.FloatTensor).to(
                        self.device)

                assert self.video_feat is not None or self.frame_feat is not None, 'Features all NONE'
