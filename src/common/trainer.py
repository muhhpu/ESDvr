...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def __init__(self, config, model, mg=False):
        ...
        self.memberStore = torch.Tensor()
        self.memberStoremasked = torch.Tensor()
    ...
    def fit(self, train_data, valid_data=None, test_data=None, saved=False, verbose=True):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if verbose:
                    # self.update_if_better(test_result)
    ...
    @torch.no_grad()
    def update_if_better(self, metric_dict):
        count = 0  # 超过最大值的指标个数
        # 判断每个指标是否超过最大值
        if metric_dict['recall@5'] > self.recall5:
            count += 1
        if metric_dict['recall@10'] > self.recall10:
            count += 1
        if metric_dict['ndcg@5'] > self.ndcg5:
            count += 1
        if metric_dict['ndcg@10'] > self.ndcg10:
            count += 1

        # 如果超过最大值的指标 ≥ 3，则更新所有最大值
        if count >= 3:
            self.recall5 = metric_dict['recall@5']
            self.recall10 = metric_dict['recall@10']
            self.ndcg5 = metric_dict['ndcg@5']
            self.ndcg10 = metric_dict['ndcg@10']
            # torch.save({'score': self.memberStore, 'masked': self.memberStoremasked}, '/home/team/caoziyi/mmrec-czy/src/store/TRY.pth')

    @torch.no_grad()
    def evaluate(self, eval_data, is_test=False, idx=0, save = False):
        r"""Evaluate the model based on the eval data.
        Returns:
        ...
        for batch_idx, batched_data in enumerate(eval_data):
            ...
            self.memberStore = scores
            # mask out pos items
            scores[masked_items[0], masked_items[1]] = -1e10
            self.memberStoremasked = scores
            # rank and get top-k
